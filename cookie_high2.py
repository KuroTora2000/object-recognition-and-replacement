    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0: 
        ...
        #for j in reversed(range(num_dets_to_consider)):
             ...
             #   print(color2)
                #print(color2)
                #if():
        #if(str(color2)!="(181, 81, 63)"or"(54, 67, 244)"):
        ...
        if (num_dets_to_consider > 1 ):#if you want to delete mask, mask here.############################################################################
            #for j in reversed(range(num_dets_to_consider)):
                #color2=get_color(j)
                #print(color2)
                #if(str(color2)!="(54, 67, 244)"):
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            if args.display_text:
                ...
                if(str(color)=="(54, 67, 244)"):
                    cv2.putText(img_numpy, " ", text_pt, font_face, font_scale1, text_color, font_thickness, cv2.LINE_AA)
                else:
                    #cv2.rectangle(img_numpy, (x1, y1), (x2, y2), color, 1)
                    #cv2.rectangle(img_numpy, (x1, y1), (x1 + text_w, y1 - text_h - 4), color, -1)#this is important.
                    #cv2.putText(img_numpy, text_str, text_pt, font_face, font_scale, text_color, font_thickness, cv2.LINE_AA)

                #for i in range(1,19):
                    img1=cv2.cvtColor(img_numpy, cv2.COLOR_BGR2GRAY)
                    trim=img1[y1:y2,x1:x2]
                    
                    #img1 = cv2.imread(img_numpy,cv2.IMREAD_GRAYSCALE)
                    #cv2.imshow(str(i)+".png",img1)
                    #print("Name:"+str(i)+".png")
                #setting value
                    m1=cv2.moments(trim)
                    #print("default value:",float(format(moments_round_angle(m1)[1])))
                    roundness=float(format(moments_round_angle(m1)[0]))
                    degree=float(format(moments_round_angle(m1)[1]))
                    #zRotation=degree
                    #print(roundness,degree)
                #adjustment value
                    '''
                    if(roundness>0.99):
                        degree =0
                    if(10<degree<89): 
                        ...
                        degree =90-(degree-90)
                #setting xRotation and yRotation
                    #print(degree)
                    
                    #if(roundness>0.90 and (60<degree<120)):
                        #xRotation=0
                        #yRotation=0
                    if(10<degree<85 or 95<degree<170):
                        xRotation=math.floor(degree)
                        yRotation = math.floor(math.degrees(math.acos(roundness)))
                    elif(0<degree<10 or 170<degree<180):
                        xRotation=0
                        yRotation = math.floor((math.degrees(math.acos(roundness))))
                    else:#angle=90
                        xRotation = math.floor(math.degrees(math.acos(roundness)))
                        yRotation=0
                        '''
                        #print("\n")
                    #cv2.putText(img_numpy, str(roundness), text_pt, font_face, font_scale, text_color, font_thickness, cv2.LINE_AA)
                    #cv2.putText(img_numpy, str(degree), text_pt1, font_face, font_scale, text_color, font_thickness, cv2.LINE_AA)
   


                    if(roundness<0.94 and (degree<3 ) ):
                        img=cv2.imread('cookie_high2_horizontal.png' ,cv2.IMREAD_UNCHANGED)
                        w = x2-x1
                        h = y2-y1
                    #dim = (width, height)
                        ...
                        img_numpy[y1:y1+height, x1:x1+width] = img_numpy[y1:y1+height, x1:x1+width] * (1 - img[:, :, 3:] / 255) + img[:, :, :3] * (img[:, :, 3:] / 255)
                    elif(roundness<0.94 and (85<degree<89 or 90<degree<95)):
                        img=cv2.imread('cookie_high2_vertical.png' ,cv2.IMREAD_UNCHANGED)
                        w = x2-x1
                        h = y2-y1
                    #dim = (width, height)
                        img = cv2.resize(img, dsize=(w, h))
                        ...
                        height, width = img.shape[:2]
                        img_numpy[y1:y1+height, x1:x1+width] = img_numpy[y1:y1+height, x1:x1+width] * (1 - img[:, :, 3:] / 255) + img[:, :, :3] * (img[:, :, 3:] / 255)
                    elif((roundness<0.94 ) and (3<degree<85  or 179.8<degree<179.9 or 89.9<degree<90 or 97.6<degree<97.7)):
                        img=cv2.imread('cookie_high2_45.png' ,cv2.IMREAD_UNCHANGED)
                        w = x2-x1
                        h = y2-y1
                    #dim = (width, height)
                        img = cv2.resize(img, dsize=(w, h))
                        ...
                        height, width = img.shape[:2]
                        img_numpy[y1:y1+height, x1:x1+width] = img_numpy[y1:y1+height, x1:x1+width] * (1 - img[:, :, 3:] / 255) + img[:, :, :3] * (img[:, :, 3:] / 255)
                    elif(roundness<0.94 and (95<degree<176 or 89.7<degree<89.8)):
                        img=cv2.imread('cookie_high2_135.png' ,cv2.IMREAD_UNCHANGED)
                        w = x2-x1
                        h = y2-y1
                    #dim = (width, height)
                        img = cv2.resize(img, dsize=(w, h))
                        ...
                        img =cv2.imread('cookie_high2.png',cv2.IMREAD_UNCHANGED)
                   # scale_percent = 200 
                        w = x2-x1
                        h = y2-y1
                    #dim = (width, height)
                    ...
                    #img_numpy2=cv2.imread(img_numpy,cv2.IMREAD_UNCHANGED)
                    #source = img_numpy
                        height, width = img.shape[:2]
                        img_numpy[y1:y1+height, x1:x1+width] = img_numpy[y1:y1+height, x1:x1+width] * (1 - img[:, :, 3:] / 255) + img[:, :, :3] * (img[:, :, 3:] / 255)
                ...
                #print(x1,y1,x2,y2,x1 + text_w,y1 - text_h - 4)
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    if is_webcam:
        ...
        vid.set(cv2.CAP_PROP_FRAME_WIDTH, 4416)
        vid.set(cv2.CAP_PROP_FRAME_HEIGHT, 1242) 
    else:
